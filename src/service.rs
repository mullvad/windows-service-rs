...
impl ServiceDependency {
    ...
    pub fn from_system_identifier(identifier: OsString) -> Self {
        let group_prefix: u16 = '+' as u16;
        let mut iter = identifier.encode_wide().peekable();
...
mod tests {
    ...
    fn test_service_group_identifier() {
        let dependency = ServiceDependency::from_system_identifier(OsString::from("+network"));
        assert_eq!(
            dependency,
    ...
    fn test_service_name_identifier() {
        let dependency = ServiceDependency::from_system_identifier(OsString::from("netlogon"));
        assert_eq!(
            dependency,
