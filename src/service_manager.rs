...
impl ServiceManager {
    ...
    fn new(
        ...
        let machine_name = to_wide(machine).map_err(|e| Error::InvalidMachineName(e.into()))?;
        let database_name = to_wide(database).map_err(|e| Error::InvalidDatabaseName(e.into()))?;
        let handle = unsafe {
    ...
    pub fn open_service(
        ...
        let service_name =
            WideCString::from_os_str(name).map_err(|e| Error::InvalidServiceName(e.into()))?;
        let service_handle = unsafe {
            winsvc::OpenServiceW(
                self.manager_handle.raw_handle(),
                service_name.as_ptr(),
