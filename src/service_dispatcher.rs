...
macro_rules! define_windows_service {
    ($function_name:ident, $service_main_handler:ident) => {
        ...
        extern "system" fn $function_name(
            ...
            raw_service_arguments: *mut *mut u16,
        ) {
            let arguments = unsafe {
                $crate::service_dispatcher::parse_raw_service_arguments(
                    num_service_arguments,
                    raw_service_arguments,
                )
            };
...
#[doc(hidden)]
pub unsafe fn parse_raw_service_arguments(argc: u32, argv: *mut *mut u16) -> Vec<OsString> {
    (0..argc)
        .into_iter()
        .map(|i| {
